...
public class Database {
    ...
    public ViewResults getDocumentRevisions(Set<String> ids) {
        ViewResults results = null;
        JSONObject json = new JSONObject();
        json.element("keys", ids);
        CouchResponse resp = session.post(name + "/_all_docs", json.toString());
        if (resp.isOk()) {
            results = new ViewResults(new View("_all_docs"), resp.getBodyAsJSONObject());
        }
        return results;
    }
    ...
    public ViewResults adhoc(final AdHocView view) {
        ...
        String adHocBody = new JSONStringer().object().key("map").value(JSONUtils.stringSerializedFunction(view.getFunction())).endObject().toString();
